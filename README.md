...
Copy `.env.example` to `.env.local` for local development and fill in the values you need:
...
The Firebase web config for the `hamroh-uz` project is already included in the codebase, so Firebase does not require Vercel environment variables for the current project. These optional variables can override the built-in config if you ever move the app to another Firebase project:
...
4. Add required server/API environment variables from `.env.example` in Vercel Project Settings.
5. Deploy.
